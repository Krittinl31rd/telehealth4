...
const Profile = () => {
  ...
  const [userImage, setUserImage] = useState(
    `${import.meta.env.VITE_API_URL}${user?.profile_image_url}`
  );
  const [authenImage, setAuthenImage] = useState(
    `${import.meta.env.VITE_API_URL}${user?.auth_image_url}`
  );

  const [userImageFile, setUserImageFile] = useState(null);
  const [authenImageFile, setAuthenImageFile] = useState(null);
  ...
  const handleSave = async () => {
    ...
    // Add images only if new file selected
    if (userImageFile != null) {
      form.append("image1", userImageFile);
    }
    if (authenImageFile != null) {
      form.append("image2", authenImageFile);
    }
            ...
            <div className="lg:col-span-3">
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
                ...
                <div className="md:col-span-2 mt-4">
                  <button
                    onClick={handleSave}
                    className="btn btn-lg btn-block border-base-content/30 bg-primary text-base-100 shadow-md"
                  >
                    Save Profile
                  </button>
                </div>
